...
class IHDPLoader(DataLoader):
    ...
    def __getitem__(self, idx):
        if not self.loaded:
            self.load()
        return self.X_tr[:, :, idx], self.T_tr[:, idx], self.YF_tr[:, idx], self.YCF_tr[:, idx], self.mu_0_tr[:,
                                                                                                 idx], self.mu_1_tr[:,
                                                                                                       idx], \
            self.X_te[:, :, idx], self.T_te[:, idx], self.YF_te[:, idx], self.YCF_te[:, idx], self.mu_0_te[:,
                                                                                              idx], self.mu_1_te[:, idx]
