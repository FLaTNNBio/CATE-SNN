...
class BCAUSS(Model, nn.Module):
    ...
    def forward(self, x):
        ...
        # Puoi aggiungerlo dopo il metodo forward()

    def embed(self, x):
        """
        Restituisce solo la rappresentazione latente (embedding) dall'input x.
        """
    ...
    def predict_propensity(self, X: np.ndarray) -> np.ndarray:
        ...
        with torch.no_grad():
            z = self.repr_net(X_t)
            p = self.t_head(z)  # output sigmoid → p ∈ [0,1]
    ...
    def mu_and_embedding(self, x):
        ...
        y0 = self.y0_net(z)
        y1 = self.y1_net(z)
        # NON concatenare y0 e y1. Restituiscili separatamente.
        return y0, y1, z  # <-- SOLUZIONE: Restituisce 3 valori
